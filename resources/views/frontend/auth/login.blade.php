...
<body class="bg-[#f1f5f9] h-screen w-screen overflow-hidden">
    ...
    <main class="flex h-full w-full">
        ...
        <section class="w-full lg:w-[45%] h-full flex flex-col justify-center px-8 md:px-16 bg-white relative z-10 shadow-2xl overflow-y-auto no-scrollbar">

            <div class="mb-8">
                <a href="{{ route('home') }}" class="inline-flex items-center gap-2 mb-6 group">
                    <div class="w-12 h-12 bg-slate-900 rounded-xl flex items-center justify-center shadow-lg group-hover:bg-[#16a34a] transition-all">
            ...
            @if (session('status'))
                <div class="mb-4 px-4 py-3 bg-green-50 border border-green-200 rounded-xl text-green-700 text-sm font-medium">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <div class="space-y-6">
                ...
                <div class="relative flex items-center justify-center">
                    ...
                    <span class="absolute bg-white px-4 text-[9px] font-black text-slate-300 uppercase tracking-[0.3em]">Or use email</span>
                </div>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email') }}" required autocomplete="username"
                            ...
                            class="w-full bg-slate-50 border @error('email') border-red-400 bg-red-50 @else border-slate-200 @enderror rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-[#16a34a] focus:bg-white transition-all font-medium">
                        @error('email')
                            <p class="text-red-500 text-[11px] font-bold ml-1 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="space-y-1">
                        <div class="flex justify-between items-center ml-1">
                            ...
                            @if (Route::has('password.request'))
                                <a href="{{ route('password.request') }}" class="text-[9px] font-black text-[#16a34a] hover:underline uppercase tracking-widest">Forgot?</a>
                            @endif
                        </div>
                        <input id="password" type="password" name="password" required autocomplete="current-password"
                            placeholder="••••••••"
                            class="w-full bg-slate-50 border @error('password') border-red-400 bg-red-50 @else border-slate-200 @enderror rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-[#16a34a] focus:bg-white transition-all font-medium">
                        @error('password')
                            <p class="text-red-500 text-[11px] font-bold ml-1 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <button type="submit"
                        class="w-full py-4 bg-slate-900 text-white rounded-xl font-black uppercase italic tracking-widest text-xs hover:bg-[#16a34a] transition-all flex items-center justify-center gap-3 shadow-xl shadow-slate-200">
                        Authorize Entry
            ...
            <p class="mt-8 text-center text-[11px] font-bold text-slate-400 uppercase tracking-widest">
                New to the community? <a href="{{ route('user-registration') }}" class="text-[#16a34a] hover:underline ml-1">Join Now</a>
            </p>
        </section>
        ...
        <section class="hidden lg:block w-[55%] h-full relative bg-slate-900">
            ...
            <div class="absolute bottom-16 left-16 right-16">
                <div class="flex items-center gap-3 mb-4">
                    <span class="px-3 py-1 bg-[#4ade80]/20 backdrop-blur-md border border-[#4ade80]/30 text-[#4ade80] text-[10px] font-black uppercase tracking-widest rounded-full">System Status: Nominal</span>
                </div>
                ...
                <p class="text-slate-300 text-sm font-medium max-w-md leading-relaxed">
                    Join thousands of collectors and enthusiasts trading the world's most advanced electric vehicles on the BijuliCar marketplace.
                </p>
            </div>
