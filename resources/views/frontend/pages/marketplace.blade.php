            ...
            <form method="GET" action="{{ route('marketplace') }}" id="quick-form">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 items-center">
                        ...
                        <div class="w-full relative">
                            <select name="drivetrain"
                                ...
                                <option value="ev" {{ request('drivetrain') === 'ev' ? 'selected' : '' }}>EV Power
                                </option>
                                <option value="hybrid" {{ request('drivetrain') === 'hybrid' ? 'selected' : '' }}>Hybrid
                                    Sync</option>
                                <option value="petrol" {{ request('drivetrain') === 'petrol' ? 'selected' : '' }}>Petrol
                                </option>
                                <option value="diesel" {{ request('drivetrain') === 'diesel' ? 'selected' : '' }}>Diesel
                                </option>
                            </select>
                        ...
                        <div class="w-full relative">
                            <select name="location"
                                ...
                                @foreach ($locations as $loc)
                                    <option value="{{ $loc }}"
                                        {{ request('location') === $loc ? 'selected' : '' }}>{{ $loc }}</option>
                                @endforeach
                            </select>
                            <div class="absolute inset-y-0 right-6 flex items-center pointer-events-none text-slate-400">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                ...
                <div class="mt-6">
                    ...
                    <div id="advanced-panel"
                        class="{{ request()->hasAny(['brand', 'model_name', 'year_from', 'year_to', 'price_min', 'price_max']) ? '' : 'hidden' }} relative mt-6 p-8 lg:p-10 bg-[#0f172a]/90 border border-white/10 rounded-[2rem] lg:rounded-[3rem] shadow-2xl backdrop-blur-xl overflow-hidden">
                        ...
                        <div class="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
                            ...
                            <div class="space-y-3">
                                <label class="text-[10px] font-black uppercase tracking-widest text-slate-400">Model
                                    Name</label>
                                <input type="text" name="model_name" value="{{ request('model_name') }}"
                            ...
                            <div class="space-y-3 lg:col-span-2">
                                <label class="text-[10px] font-black uppercase tracking-widest text-slate-400">Price
                                    Structure (NRs)</label>
                                <div class="grid grid-cols-2 gap-3">
                                    <input type="text" name="price_min" value="{{ request('price_min') }}"
                                        placeholder="Min"
                                        class="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-4 text-sm font-bold text-white placeholder:text-slate-600 outline-none">
                                    <input type="text" name="price_max" value="{{ request('price_max') }}"
                                        placeholder="Max"
                                        class="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-4 text-sm font-bold text-white placeholder:text-slate-600 outline-none">
                                </div>
                            </div>
                            <div class="flex items-end pb-1 lg:justify-center">
                                <label class="relative inline-flex items-center cursor-pointer group">
                                    <input type="checkbox" name="only_available" value="1" class="sr-only peer"
                ...
                <div class="mt-10 flex flex-col md:flex-row items-center justify-between gap-6 px-4">
                    <div class="flex flex-wrap justify-center items-center gap-4 lg:gap-6">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span
                                class="text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest">{{ $cars->total() }}
                                Results</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['sort' => 'newest']) }}"
                            class="text-[9px] lg:text-[10px] font-black uppercase tracking-widest transition-colors {{ request('sort', 'newest') === 'newest' ? 'text-white underline decoration-[#4ade80] decoration-2 underline-offset-4' : 'text-slate-500 hover:text-white' }}">Newest</a>
                        <a href="{{ request()->fullUrlWithQuery(['sort' => 'price_asc']) }}"
                            class="text-[9px] lg:text-[10px] font-black uppercase tracking-widest transition-colors {{ request('sort') === 'price_asc' ? 'text-white underline decoration-[#4ade80] decoration-2 underline-offset-4' : 'text-slate-500 hover:text-white' }}">Price
                            ↑</a>
                        <a href="{{ request()->fullUrlWithQuery(['sort' => 'price_desc']) }}"
                            class="text-[9px] lg:text-[10px] font-black uppercase tracking-widest transition-colors {{ request('sort') === 'price_desc' ? 'text-white underline decoration-[#4ade80] decoration-2 underline-offset-4' : 'text-slate-500 hover:text-white' }}">Price
                            ↓</a>
                    </div>
                </div>
            </form>
        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            @if ($cars->isNotEmpty())
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($cars as $car)
                        <div
                            class="group bg-white rounded-[2rem] overflow-hidden shadow-[0_4px_20px_-4px_rgba(0,0,0,0.06)] hover:shadow-[0_30px_60px_-12px_rgba(0,0,0,0.12)] transition-all duration-500 border border-transparent hover:border-slate-100 flex flex-col">
                            ...
                            <div class="relative h-48 bg-slate-900 overflow-hidden">
                                ...
                                <div class="absolute top-4 left-4">
                                    <span
                                        @class([
                                            ...
                                            'bg-slate-600 text-white' => in_array($car->drivetrain, [
                                                'petrol',
                                                'diesel',
                                            ]),
                                ...
                                <div class="absolute top-4 right-4">
                                    <span
                                        class="text-[9px] font-black px-2.5 py-1 rounded-full uppercase tracking-widest bg-black/60 text-white backdrop-blur-sm">
                            ...
                            <div class="p-6 flex flex-col flex-1">
                                <div class="mb-4">
                                    <h3
                                        class="text-lg font-black text-slate-900 uppercase italic tracking-tight leading-tight">
                                        {{ $car->displayName() }}</h3>
                                ...
                                <div class="flex flex-wrap gap-2 mb-5">
                                    <span
                                        class="text-[10px] font-black px-2 py-1 bg-slate-100 text-slate-600 rounded-lg uppercase tracking-wider">{{ number_format($car->mileage) }}
                                        km</span>
                                    @if ($car->range_km)
                                        <span
                                            class="text-[10px] font-black px-2 py-1 bg-[#4ade80]/10 text-[#16a34a] rounded-lg uppercase tracking-wider border border-[#4ade80]/20">{{ $car->range_km }}
                                            km range</span>
                                    @endif
                                    @if ($car->battery_kwh)
                                        <span
                                            class="text-[10px] font-black px-2 py-1 bg-slate-100 text-slate-600 rounded-lg uppercase tracking-wider">{{ $car->battery_kwh }}
                                            kWh</span>
                                    @endif
                                </div>
                                ...
                                <div class="mt-auto pt-4 border-t border-slate-100 flex items-end justify-between">
                                    <div>
                                        <span
                                            class="block text-[10px] uppercase tracking-widest text-slate-400 font-bold">Price</span>
                                        <span
                                            class="text-xl font-black text-slate-900 italic tracking-tight">{{ $car->formattedPrice() }}</span>
                                        @if ($car->price_negotiable)
                                            <span
                                                class="block text-[9px] font-black text-[#16a34a] uppercase tracking-widest mt-0.5">Negotiable</span>
                                        @endif
                                    </div>
                                    ...
                                    @auth
                                        ...
                                        @else
                                            <a href="{{ route('login') }}"
                                                class="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-700 transition-colors">Login
                                                to Order</a>
                                        @endif
                                    @else
                                        <a href="{{ route('login') }}"
                                            class="flex items-center gap-2 bg-slate-100 text-slate-700 px-4 py-2.5 rounded-xl text-[11px] font-black uppercase italic tracking-widest hover:bg-slate-900 hover:text-white transition-all">Login
                                            to Order</a>
                                    @endauth
                                </div>
                            </div>
            ...
            @else
                <div class="text-center py-20">
                    ...
                    <h3 class="text-2xl font-black text-slate-900 uppercase italic tracking-tight mb-3">No vehicles found
                    </h3>
                    <p class="text-slate-500 font-medium mb-8">Try adjusting your search or filters.</p>
                    <a href="{{ route('marketplace') }}"
                        class="inline-flex items-center gap-2 bg-slate-900 text-white px-6 py-3 rounded-xl text-[12px] font-black uppercase italic tracking-widest hover:bg-[#16a34a] transition-all shadow-lg">Clear
                        Filters →</a>
                </div>
            @endif
