...
<body class="bg-[#f1f5f9] h-screen w-screen overflow-hidden">
    ...
    <main class="flex h-full w-full">
        ...
        <section class="w-full lg:w-[45%] h-full flex flex-col justify-center px-8 md:px-16 bg-white relative z-10 shadow-2xl overflow-y-auto no-scrollbar">
            ...
            @if (session('status'))
                <div class="mb-4 font-medium text-sm text-green-600 border border-green-200 bg-green-50 px-4 py-3 rounded-xl">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <div class="space-y-6">
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-4">
                    ...
                    <div class="space-y-1">
                        ...
                        @if ($errors->has('email'))
                            <p class="mt-1 text-[11px] text-red-600 font-bold uppercase italic tracking-wider">{{ $errors->first('email') }}</p>
                        @endif
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        @if ($errors->has('password'))
                            <p class="mt-1 text-[11px] text-red-600 font-bold uppercase italic tracking-wider">{{ $errors->first('password') }}</p>
                        @endif
                    </div>

                    <div class="flex items-center gap-2 px-1 py-1">
                        <input type="checkbox" id="remember_me" name="remember" class="w-4 h-4 rounded border-slate-300 text-[#16a34a] focus:ring-[#4ade80]">
                        <label for="remember_me" class="text-[11px] font-bold text-slate-500 cursor-pointer">Stay signed in</label>
                    </div>

                    <button type="submit" class="w-full py-4 bg-slate-900 text-white rounded-xl font-black uppercase italic tracking-widest text-xs hover:bg-[#16a34a] transition-all flex items-center justify-center gap-3 shadow-xl shadow-slate-200 group">
                        Authorize Entry
            ...
            <p class="mt-8 text-center text-[11px] font-bold text-slate-400 uppercase tracking-widest">
                New to the community? <a href="{{ route('register') }}" class="text-[#16a34a] hover:underline ml-1">Join Now</a>
            </p>
        </section>
