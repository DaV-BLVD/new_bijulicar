...
<div class="flex min-h-screen">
    ...
    <aside class="w-60 bg-slate-900 flex flex-col fixed inset-y-0 z-40">
        ...
        <div class="px-5 py-5 border-b border-slate-700">
            <a href="{{ route('home') }}" class="flex items-center gap-2">
                ...
                <span class="text-white font-black tracking-tighter uppercase text-sm">Bijuli<span class="text-[#a855f7]">Car</span></span>
            </a>
        ...
        <div class="px-5 py-4 border-b border-slate-700">
            ...
            <span class="inline-block mt-1 text-[10px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider bg-purple-500/10 text-purple-400 border border-purple-500/20">
                Business
            </span>
        </div>
        ...
        <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
            ...
            <a href="{{ route('business.cars.index') }}"
                ...
                @if($totalCars > 0)
                    <span class="ml-auto text-[10px] bg-slate-700 text-slate-300 px-1.5 py-0.5 rounded-full font-black">{{ $totalCars }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('business.orders.index') }}"
                ...
                @if($pendingOrders > 0)
                    <span class="ml-auto text-[10px] bg-yellow-500/20 text-yellow-400 border border-yellow-500/20 px-1.5 py-0.5 rounded-full font-black">{{ $pendingOrders }}</span>
                @endif
            </a>
    ...
    <div class="flex-1 ml-60">
        ...
        {{-- Flash messages --}}
        @if(session('success'))
            <div class="mx-8 mt-5 bg-purple-500/10 border border-purple-500/30 text-purple-700 rounded-xl px-4 py-3 text-sm font-bold">
                {{ session('success') }}
            </div>
        @endif
        @if(session('error'))
            <div class="mx-8 mt-5 bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-sm font-bold">
                {{ session('error') }}
            </div>
        @endif
