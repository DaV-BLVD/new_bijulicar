...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <a href="{{ route('buyer.orders.index') }}" class="bg-white border border-slate-200 rounded-2xl p-5 hover:shadow-md hover:border-slate-300 transition-all block group">
        ...
        <a href="{{ route('buyer.purchases.index') }}" class="bg-white border border-slate-200 rounded-2xl p-5 hover:shadow-md hover:border-slate-300 transition-all block group">
            <div class="flex items-center justify-between mb-3">
                <div class="w-9 h-9 bg-[#4ade80]/10 border border-[#4ade80]/20 rounded-xl flex items-center justify-center">
                    <svg class="w-5 h-5 text-[#16a34a]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                </div>
                <svg class="w-4 h-4 text-slate-300 group-hover:text-slate-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            </div>
        ...
        @can('write reviews')
        <a href="{{ route('buyer.reviews.index') }}" class="bg-white border border-slate-200 rounded-2xl p-5 hover:shadow-md hover:border-slate-300 transition-all block group">
            <div class="flex items-center justify-between mb-3">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-6">
        <div class="flex items-center justify-between mb-5">
            ...
            <a href="{{ route('buyer.orders.index') }}" class="text-[10px] font-black text-[#16a34a] uppercase tracking-widest hover:underline">View All →</a>
        </div>
        <div class="space-y-3">
            @foreach($recentOrders as $order)
            <div class="flex items-center justify-between py-3 border-b border-slate-100 last:border-0">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 bg-slate-100 rounded-xl flex items-center justify-center text-[10px] font-black text-slate-500 uppercase">EV</div>
                    <div>
                        <p class="text-sm font-black text-slate-900">{{ $order->car->displayName() }}</p>
                        <p class="text-[11px] text-slate-400 font-medium mt-0.5">{{ $order->ordered_at->diffForHumans() }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <p class="text-sm font-black text-slate-700 hidden sm:block">{{ $order->car->formattedPrice() }}</p>
                    <span @class([
    ...
    <div class="bg-white border border-dashed border-slate-200 rounded-2xl p-10 text-center">
        ...
        <a href="{{ route('marketplace') }}" class="inline-flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-xl text-[12px] font-black uppercase italic tracking-widest hover:bg-[#16a34a] transition-all">
            Browse Marketplace →
        </a>
    </div>
    @endif
